...
if isfile(join(dirname(__file__), 'sklearn_utils.py')):
...
class Protein_Landscape():
    ...
    def __init__(self,
        ...
        if data is None and not csv_path:
        ...
        self.tokens = {x: y for x, y in zip(
            self.amino_acids, list(range(len(self.amino_acids))))}
        ...
        self.token_dict = {tuple(seq): idx for idx,
                           seq in enumerate(self.tokenized[:, :-1])}
        ...
        # self.minima, self.maxima = self.get_extrema()
        # self.num_minima,self.num_maxima = self.minima.shape[0], self.maxima.shape[0]
    ...
    def calc_mutated_positions(self):
        ...
        """
        mutated_bools = np.invert(np.all(self.tokenized[:, :-1] == self.tokenize(
            self.seed_seq), axis=0))  # Calculates the indices all of arrays which are modified.
    ...
    def sklearn_data(self, data=None, distance=None, positions=None, split=0.8, shuffle=True):
        ...
        if data is not None:
            ...
            data = copy.copy(self.get_mutated_positions(
                positions, tokenize=True))
        else:
    ...
    def lengthen_sequences(self, seq_len, AAs=None, mut_indices=False):
        ...
        lengthened_data = np.zeros(
            (sequences.shape[0], seq_len), dtype=np.int8)
        ...
        if mut_indices.__class__.__name__ == "ndarray":
            assert len(
                mut_indices) == sequences.shape[1], "The index array must have the same length as the original sequence"
            idxs = mut_indices
    ...
    def is_extrema(self, idx, graph=None):
        ...
        max_comparisons = np.greater(
            self.fitnesses[idx], self.fitnesses[neighbours])
        min_comparisons = np.less(
            self.fitnesses[idx], self.fitnesses[neighbours])
    ...
    def calc_neighbours(self, seq, token_dict=None):
        ...
        actual_neighbours = [token_dict[tuple(
            key)] for key in possible_neighbours if tuple(key) in token_dict]
        return seq, actual_neighbours

    def get_extrema(self, idxs=None):
        if idxs is None:
        ...
        minima = idxs[np.where(results == -1)[0]]
        maxima = idxs[np.where(results == 1)[0]]
    ...
    def build_graph(self, idxs=None):
        ...
        if idxs is None:
            ...
            token_dict = {key: value for key, value in self.token_dict.items(
    ...
    def indexing(self, distances=None, percentage=None, positions=None):
        ...
        if positions is not None:
            ...
            not_positions = [x for x in range(
                len(self.seed_seq)) if x not in positions]
            working = reduce(
                np.logical_or, [self.sequence_mutation_locations[:, pos] for pos in positions])
            for pos in not_positions:
                temp = np.logical_xor(
                    working, self.sequence_mutation_locations[:, pos])
                working = np.logical_and(temp, np.logical_not(
    ...
    def get_optima(self, top_n=None, tops=None):
        if tops is None:
            if top_n is None:
                tops = self.data
            else:
                tops = self.data[self.fitnesses.argsort()[-top_n:]]
        if top_n is None:
            top_n = len(tops)
        ###
        onehot = np.array(
            [s_utils.string_to_one_hot(seq, self.amino_acids)
             for seq in tops[:, 0]],
            dtype=np.float32,
        ).reshape(top_n, -1)
        ###
        ...
        n_component = np.where(cov <= 0.95)[0][-1]
        ###
        distance = np.array([[self.hamming(str1, str2)
                            for str1 in tops] for str2 in tops[:, 0]])
        kmean = KMeans(n_clusters=n_component,
                       precompute_distances=distance).fit(onehot)
        ###
        clusters = pd.DataFrame(tops, columns=['seq', 'score'])
        clusters['label'] = kmean.labels_
        clusters = clusters.sort_values('score', ascending=False).groupby(
            'label', as_index=False).first()
        return clusters.loc[:, ['seq', 'score']]
